"""Train residual MLP predictors at longer horizons (DA9 Exp 3).

Fork of ``train_mlp_residual.py`` with a ``--horizon`` parameter.
Predicts ``delta_z = z_future - z_t`` in normalized space.

Usage::

    python scripts/train_mlp_residual_horizon.py --horizon 8
    python scripts/train_mlp_residual_horizon.py --horizon 16 --encoders vit_s16 clip_b32
"""
...
def _output_root(horizon: int) -> Path:
    return Path(f"outputs/latent_predictors_residual_h{horizon}")
...
def _train_one(
    ...
    """Train a single residual MLP predictor at the given horizon."""
    variant = "conditioned"
    ...
    checkpoint = {
        ...
        "provenance": {
            ...
            "seed": seed,
            "horizon": horizon,
            "source": "scripts/train_mlp_residual_horizon.py",
        },
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Train residual MLP predictors at longer horizons (DA9 Exp 3)."
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print(f"DA9 Exp 3: Residual MLP Training (horizon={horizon})")
    ...
    for enc in encoders:
        ...
        for seed in seeds:
            print(f"\n  --- {enc} / seed={seed} / h={horizon} (residual) ---")
