...
def build_fig3_cossim_lines(cossim_df: pd.DataFrame, has_ci: bool, out_dir: Path) -> None:
    ...
    if has_ci:
        ax.set_title("CosSim by Prediction Horizon (3 seeds, 95% CI)", fontsize=12)
        caption = "trainval-mirror subset (180/20/40, seed 42)\nn=3 seeds, 95% bootstrap CI"
    else:
...
def build_fig4_delta_bars(cossim_df: pd.DataFrame, has_ci: bool, out_dir: Path) -> None:
    ...
    # Title and caption adapt to data format
    if has_ci:
        ax.set_title("DeltaCosSim by Prediction Horizon (3 seeds, 95% CI)", fontsize=12)
        caption = "trainval-mirror subset (180/20/40, seed 42)\nn=3 seeds, 95% bootstrap CI"
    else:
