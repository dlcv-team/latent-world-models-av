"""Decode VAE-latent DiT predictions to images for paper figure."""
...
def run_visual_eval(n_windows: int = 20, seed: int = 0, horizon: int = 16):
    ...
    with torch.no_grad():
        for wi, frame_i in enumerate(picks):
            ...
            for col, k in enumerate(STEPS_SHOW):
                ...
                axes[0, col].set_title(f"GT t+{k}")
                axes[0, col].axis("off")
                axes[1, col].imshow(pr_img)
                axes[1, col].set_title(f"DiT t+{k}")
                axes[1, col].axis("off")
    ...
    return {
        "n_windows": len(picks),
        "mean_1_minus_mse": float(np.mean(proxy_scores)) if proxy_scores else 0.0,
        "pdf": pdf_path,
    }
