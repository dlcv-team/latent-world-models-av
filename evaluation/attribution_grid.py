...
Creates 5×4 grid figure PDF from per-encoder attribution outputs (B7).
...
class AttributionGridGenerator:
    """Generates 5×4 grid figure from per-encoder attribution outputs.

    Creates a publication-ready grid layout with:
    - 5 rows (one per encoder: ViT-S/16, DINOv2-S/14, CLIP ViT-B/32, VQ-VAE, V-JEPA2)
    ...
    # Encoder display names in order
    ENCODERS = [
        ("vit_s16", "ViT-S/16"),
    ...
    def load_images(self) -> Dict[Tuple[str, str], np.ndarray]:
        ...
        for encoder_key, _ in self.ENCODERS:
    ...
    def _discover_available_scenarios(self) -> List[str]:
        ...
        # Extract scenario names from filenames: {encoder}_{scenario}_{index}.png
        discovered_scenarios = set()
        encoder_keys = [k for k, _ in self.ENCODERS]

        for filepath in png_files:
            filename = filepath.name
            # Expected pattern: {encoder}_{scenario}_{index:02d}.png
            # Remove .png extension and split off index (last component)
            name_without_ext = filename.replace('.png', '')
            parts = name_without_ext.rsplit('_', 1)  # Split off index
            if len(parts) != 2:
                continue

            prefix, _ = parts  # prefix is {encoder}_{scenario}

            # Try to match against known encoder keys
            # Since encoder names can contain underscores (vit_s16), we need to
            # check if the prefix starts with any known encoder
            for encoder_key in encoder_keys:
                if prefix.startswith(encoder_key + '_'):
                    # Extract scenario: everything after encoder_key + '_'
                    scenario_key = prefix[len(encoder_key) + 1:]
                    if scenario_key:  # Make sure scenario is not empty
    ...
    def create_grid_figure(self) -> Tuple[plt.Figure, np.ndarray]:
        ...
        - Rows 1-5: Encoder rows (one per encoder)
        ...
        axes
            2D numpy array of Axes objects, shape (6, 4).
        """
        # Create figure with appropriate size for 5×4 grid at 300 DPI
        fig = plt.figure(figsize=(16, 20), dpi=self.dpi)

        # GridSpec: 6 rows (header + 5 encoders) × 4 columns
        gs = gridspec.GridSpec(
            6, 4,
            figure=fig,
            height_ratios=[0.3, 1, 1, 1, 1, 1],  # Header row smaller
    ...
    def generate_caption(self) -> str:
        ...
        if report_path.exists():
            try:
                ...
                vqvae_info = report.get("encoders", {}).get("vqvae", {})
                if vqvae_info.get("fallback_used", False):
                    # Add fallback caveat
                    vq_caveat = (
                        "VQ-VAE used DINOv2-S/14 fallback (checkpoint unavailable); "
                        "vqvae row does not represent independent VQ performance."
                    )
    ...
    def generate(self) -> Path:
        ...
        # Populate encoder rows (rows 1-5)
        for row_idx, (encoder_key, encoder_name) in enumerate(self.ENCODERS, start=1):
            # Add encoder label on the left
            fig.text(
                0.02,  # Far left
                1 - (row_idx + 0.5) / 6,  # Vertical position (centered in row)
                encoder_name,
                ha='left',
                va='center',
                fontsize=12,
...
def get_encoder_display_name(encoder_key: str) -> str:
    ...
    mapping = {
        ...
        "vqvae": "VQ-VAE",
        "vjepa2": "V-JEPA2",
        "vjepa2_rep1": "V-JEPA2 (1-frame)",
    }
    return mapping.get(encoder_key, encoder_key)
