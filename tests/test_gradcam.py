...
def mock_dinov2_encoder():
    ...
    # Set up last block's attention module to match real DINOv2-S/14 structure
    attn = MagicMock()
    attn.qkv = MagicMock(spec=torch.nn.Linear)  # Hook target: QKV projection layer
    attn.num_heads = 6  # DINOv2-S/14 has 6 heads
    attn.scale = 0.125  # 1/sqrt(64) where head_dim = 384/6 = 64
    encoder.backbone.blocks[-1].attn = attn

    # Mock image normalization stats (required by DINOv2Attribution)
    encoder._image_mean = torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1)
    encoder._image_std = torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1)
...
def mock_vqvae_encoder():
    """Mock VQ-VAE encoder with fallback active (simulates checkpoint load failure)."""
    encoder = MagicMock()
    encoder.fallback_active = True
    encoder.backbone = MagicMock()
    encoder.backbone.blocks = [MagicMock() for _ in range(12)]

    # Set up DINOv2 attention structure for fallback path
    attn = MagicMock()
    attn.qkv = MagicMock(spec=torch.nn.Linear)  # Hook target
    attn.num_heads = 6
    attn.scale = 0.125
    encoder.backbone.blocks[-1].attn = attn

    # Mock image normalization stats
    encoder._image_mean = torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1)
    encoder._image_std = torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1)
...
class TestAttributionMethods:
    ...
    def test_dinov2_attribution_shape(self, mock_dinov2_encoder):
        """Test DINOv2 attribution output shape with QKV hook."""
        method = DINOv2Attribution(mock_dinov2_encoder, device="cpu")

        # Captured QKV output that the hook will inject
        # Shape: [B, N, 3*C] = [1, 257, 1152] for DINOv2-S/14
        # 257 tokens = 1 CLS + 256 patches (16x16 grid for 224px/14)
        # 1152 = 3 * 384 (Q, K, V projections concatenated)
        mock_qkv_output = torch.randn(1, 257, 1152)

        # Hook container
        captured_hooks = []

        def mock_register_hook(hook_fn):
            """Mock register_forward_hook to inject QKV output."""
            captured_hooks.append(hook_fn)
            # Simulate the hook being called during forward pass
            # hook signature: fn(module, input, output)
            hook_fn(None, None, mock_qkv_output)
            # Return mock handle
            handle = MagicMock()
            handle.remove = MagicMock()
            return handle

        # Patch the QKV layer's register_forward_hook method
        mock_dinov2_encoder.backbone.blocks[-1].attn.qkv.register_forward_hook = mock_register_hook

        # Mock backbone forward to avoid actual model execution
        mock_dinov2_encoder.backbone.eval = MagicMock()

        input_tensor = torch.randn(1, 3, 224, 224)

        # Should now successfully extract attention and return valid attribution
        attribution = method.compute_attribution(input_tensor)

        assert attribution.shape == (224, 224), f"Expected (224, 224), got {attribution.shape}"
        assert attribution.dtype == np.float32
        # Verify attribution is normalized [0, 1]
        assert attribution.min() >= 0.0 and attribution.max() <= 1.0
    ...
    def test_vqvae_fallback_detection(self, mock_vqvae_encoder):
        ...
        # Mock QKV hook for DINOv2 fallback path
        mock_qkv_output = torch.randn(1, 257, 1152)
        ...
        mock_vqvae_encoder.backbone.blocks[-1].attn.qkv.register_forward_hook = mock_register_hook
        mock_vqvae_encoder.backbone.eval = MagicMock()
    ...
    def test_vjepa_temporal_input(self, mock_vjepa_encoder):
        """Test V-JEPA handles temporal input (16-frame clip)."""
        method = VJEPA2Attribution(mock_vjepa_encoder, device="cpu")

        # Mock backbone hook to inject features
        # V-JEPA2 ViT-L/16 at 256x256 with tubelet=2:
        # num_tokens = (T/2) * (256/16)^2 = (16/2) * 16^2 = 8 * 256 = 2048
        mock_features = torch.randn(1, 2048, 1024)

        def mock_register_hook(hook_fn):
            """Mock register_forward_hook to inject features."""
            hook_fn(None, None, mock_features)
            handle = MagicMock()
            handle.remove = MagicMock()
            return handle

        mock_vjepa_encoder.backbone.register_forward_hook = mock_register_hook
    ...
    def test_vjepa_single_frame_input(self, mock_vjepa_encoder):
        """Test V-JEPA rep1 single-frame ablation (spatial-only path)."""
        method = VJEPA2Attribution(mock_vjepa_encoder, device="cpu")

        # Mock backbone hook to inject features
        # Single-frame with tubelet=2: only spatial tokens, no temporal grouping
        # num_tokens = (256/16)^2 = 16^2 = 256
        mock_features = torch.randn(1, 256, 1024)
        ...
        # Single-frame input: (B, T=1, 3, H, W)
        input_tensor = torch.randn(1, 1, 3, 224, 224)
        attribution = method.compute_attribution(input_tensor)
        assert attribution.shape == (224, 224)
        assert attribution.min() >= 0.0 and attribution.max() <= 1.0
...
class TestAttributionPipeline:
    ...
    def test_overlay_generation(self):
        ...
        # Title should contain encoder display name and scenario
        assert ENCODER_DISPLAY["vit_s16"] in title  # "ViT-S/16"
