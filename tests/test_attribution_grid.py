...
class TestAttributionGridGenerator:
    ...
    def temp_input_dir(self, tmp_path):
        ...
        # Use all encoders from AttributionGridGenerator.ENCODERS (M1 canonical keys)
        encoders = AttributionGridGenerator.ENCODERS
    ...
    def test_load_images(self, temp_input_dir, tmp_path):
        ...
        # Should load len(ENCODERS) × 2 discovered scenarios
        n_encoders = len(AttributionGridGenerator.ENCODERS)
        n_scenarios = 2
        assert len(images) == n_encoders * n_scenarios
        # Verify dynamic discovery worked
        assert generator._discover_available_scenarios() == ["intersection", "other"]

        # Check specific keys (using M1 canonical keys)
        assert ("vit_s16", "intersection") in images
        assert ("vit_s16", "other") in images
        assert ("dino_vits14", "intersection") in images
        assert ("vjepa2_rep1", "intersection") in images
    ...
    def test_create_grid_figure(self, tmp_path):
        ...
        # Check axes shape: (len(ENCODERS) + 1) rows × 4 columns
        n_rows = len(AttributionGridGenerator.ENCODERS) + 1
        n_cols = 4
    ...
    def test_generate_caption_no_fallback(self, tmp_path):
        ...
        # Create method report with fallback_used=False (using M1 canonical key)
        report = {
            "encoders": {
                "vq_track": {
                    "fallback_used": False
    ...
    def test_generate_caption_with_fallback(self, tmp_path):
        ...
        # Create mock figures_method_report.json with fallback (using M1 canonical key)
        report = {
            "encoders": {
                "vq_track": {
                    "fallback_used": True
