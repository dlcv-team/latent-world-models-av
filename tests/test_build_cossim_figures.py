...
def test_fig3_caption_multiseed(tmp_path):
    """Fig 3 multi-seed caption includes 3 seeds + bootstrap CI."""
    csv_path = _mock_3seed_csv(tmp_path)
    df, has_ci = bcf.load_cossim_csv(csv_path)
    out_dir = tmp_path / "figures"
    out_dir.mkdir()

    # Mock ax.text to capture caption
    with mock.patch("matplotlib.pyplot.subplots") as mock_subplots:
        ...
        # Check ax.text was called with required caption strings
        text_calls = [str(call) for call in mock_ax.text.call_args_list]
        text_str = " ".join(text_calls)
        assert "trainval-mirror subset (180/20/40, seed 42)" in text_str
        assert "3 seeds" in text_str
        assert "95% bootstrap CI" in text_str
...
def test_fig3_caption_singleseed(tmp_path):
    ...
    with mock.patch("matplotlib.pyplot.subplots") as mock_subplots:
        ...
        text_calls = [str(call) for call in mock_ax.text.call_args_list]
        text_str = " ".join(text_calls)
        assert "single-seed run (no CI)" in text_str
        assert "3 seeds" not in text_str
